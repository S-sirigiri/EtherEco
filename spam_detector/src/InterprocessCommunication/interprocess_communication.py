...
class InterprocessCommunicationSharedMemory:
    ...
    def __init__(self, shared_memory_name: str, semaphore_name):
        self.sharedMemoryName = shared_memory_name
        self.semaphoreName = semaphore_name
    ...
    def send(self, message: str) -> None:
        ...
        map_file.write(message.encode('utf-8'))

        # Close the mapfile. The shared memory segment will persist until it's deleted.
        map_file.close()

        # Create a semaphore
        semaphore = posix_ipc.Semaphore(self.semaphoreName, posix_ipc.O_CREAT)
...
class InterprocessCommunicationMessageQueue:
    ...
    def closeMessageQueue(self) -> None:
        # Close the queue
        self.mq.close()
